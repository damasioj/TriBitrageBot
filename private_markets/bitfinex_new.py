...
def query_order(amount, price, symbol, _type):
    ...
    return client.post('/v2/order/orders/place',  {
...
def query_transfer(_from, to, currency, amount):
    client = ApiClient(ACCESS_KEY, SECRET_KEY)
    return client.post('/v2/dw/balance/transfer',  {
            'from' : _from,
            'to' : to,
            'currency' : currency,
            'amount' : amount,
...
def get_account():
    client = ApiClient(ACCESS_KEY, SECRET_KEY)
    return client.get('/v2/account/accounts')

def get_account_info():
    client = ApiClient(ACCESS_KEY, SECRET_KEY)
    return client.get('/v2/account/accounts/' + str(ACCOUNT_ID) + '/balance')  
